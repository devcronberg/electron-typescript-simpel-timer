...
setInterval(function () {
    var diff = indstillinger.til.diff(moment());
    ...
    if (diff <= 0) {
        ...
        $("section#timer2  .value").text(byteString(parseInt(moment.utc(diff).format("mm"))) +
            ":" +
            byteString(parseInt(moment.utc(diff).format("ss"))));
        $("section#timer16  .value").text(pad(hexString(parseInt(moment.utc(diff).format("mm"))), 2) +
            ":" +
            pad(hexString(parseInt(moment.utc(diff).format("ss"))), 2));
        $("section#timer8 .value").text(pad(octalString(parseInt(moment.utc(diff).format("mm"))), 2) +
            ":" +
...
electron_1.ipcRenderer.on("menu", function (e, d) {
    $("section").hide();
...
$("#gem").click(function () {
    indstillinger.tid = parseInt($("#tid")
        .val()
        .toString());
    indstillinger.til = moment().add(indstillinger.tid, "minutes");
    $("body").css("animation-name", "colorWhite");
